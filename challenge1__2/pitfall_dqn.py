...
-----
  ...
  python pitfall_dqn.py --mode train --model-path models/pitfall_dqn

  # Train a specific named experiment from the JSON config
  python pitfall_dqn.py --mode train --experiment exp_02_lr_high --model-path models/pitfall_dqn

  # Watch the trained agent play
  python pitfall_dqn.py --mode play --model-path models/pitfall_dqn --episodes 3

  # Run all experiments from sweep_configs.json and keep the best model
  python pitfall.py --mode sweep --sweep-file sweep_configs.json
...
class TensorBoardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self._writer.add_scalar(
            "training/epsilon", self.model.exploration_rate, self.num_timesteps
        )

        if self.locals["dones"][0]:
            self._writer.add_scalar(
                "training/episode_reward", self._episode_reward, self.num_timesteps
            )
            self._episode_reward = 0.0

        return True  # returning False would abort training

# Curiosity Wrapper: A simple count-based exploration bonus that encourages the agent to visit novel states.
...
# A simple wrapper to demonstrate reward shaping by penalizing inactivity and repeated states.
...
class RewardShapingWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # Penalizar no cambiar el estado
        if self.last_obs is not None and (obs == self.last_obs).all():
            reward -= 0.01
...
def build_training_environment(
    seed: int, use_curiosity: bool = False, curiosity_beta: float = 0.01
) -> VecFrameStack:
    ...
    env = VecMonitor(env)  # Adds episode reward and length info to the 'info' dict
...
def inspect_model(model_path: str) -> None:
    ...
    Example:
        python pitfall_dqn.py --mode inspect --model-path models/pitfall_dqn
    """
    if not os.path.exists(f"{model_path}.zip"):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        choices=["train", "play", "inspect", "sweep"],
        required=True,
